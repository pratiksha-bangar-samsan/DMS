...
connections=self.mp_face_mesh.FACEMESH_LIPS,
            ...
            self.metrics['faces_detected'] = faces_detected
...
def start_event_detection():
    """
    This function can be called to start the event detection.
    It encapsulates the logic that was previously in the __main__ block.
    """
    parser = argparse.ArgumentParser(description="Run the Samsan DMS Monitor.")
    parser.add_argument("--roi", type=str, default="DEFAULT", help="Specify the ROI position from config file (e.g., 'TOP', 'BOTTOM').")
    args = parser.parse_args()
    
    # Instantiate and run your monitoring class here.
    monitor = EdgetensorDMMMonitor(args.roi)
    monitor.run()
