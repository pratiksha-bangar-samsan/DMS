            ...
            if not is_camera_obstructed:
                ...
                if results.multi_face_landmarks:
                    ...
                    for face_landmarks in results.multi_face_landmarks:
                        ...
                        for index in self.NOSE_LANDMARKS:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--roi", type=str, default=None,
                        help="Specify the ROI position (e.g., 'left', 'right', 'center'). Overrides config.")
    args = parser.parse_args()
    
    # Check if an ROI was provided via command line. If not, use the default from the config file.
    if args.roi is None:
        config = configparser.ConfigParser()
        config.read('config.ini')
        try:
            active_roi_from_config = config.get('DEFAULT', 'active_roi')
            args.roi = active_roi_from_config
        except (configparser.NoSectionError, configparser.NoOptionError) as e:
            print(f"Warning: Could not read default ROI from config file. Using 'center'. Error: {e}")
            args.roi = 'center'
