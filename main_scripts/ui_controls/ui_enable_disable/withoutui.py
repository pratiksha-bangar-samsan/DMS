...
# without a graphical user interface (GUI). It's intended to be run
# when the `ui_enable` setting in `config.ini` is False.
...
class EdgetensorDMMMonitor:
    ...
    def update_event_states(self, yawn_detected, distraction_detected, head_direction, left_eye_closed, right_eye_closed, is_camera_obstructed, faces_detected):
        ...
        self.fatigue_duration += delta_time if left_eye_closed and right_eye_closed else -delta_time * 2
        ...
        self.left_eye_closed_duration += delta_time if left_eye_closed else -delta_time * 2
        self.right_eye_closed_duration += delta_time if right_eye_closed else -delta_time * 2
        ...
        self.left_eye_closed_duration = max(0, self.left_eye_closed_duration)
        self.right_eye_closed_duration = max(0, self.right_eye_closed_duration)
