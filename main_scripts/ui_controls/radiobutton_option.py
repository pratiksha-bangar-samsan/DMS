...
# ==================== GUI and Program Control ======================
class DMS_Application:
    def __init__(self, master):
        self.master = master
        master.title("DMS and Mask Detection App")
        ...
        selection_frame.pack(side=tk.RIGHT, padx=10, pady=10, fill=tk.Y)
        
        tk.Label(selection_frame, text="Select Program to Run:", font=("Helvetica", 12, "bold")).pack(pady=5)
        
        tk.Radiobutton(selection_frame, text="Mask Detection (Video File)", variable=self.selected_option, value=1, command=self.stop_and_select).pack(anchor="w", pady=2)
        tk.Radiobutton(selection_frame, text="Other Events (DMS) (USB Camera)", variable=self.selected_option, value=2, command=self.stop_and_select).pack(anchor="w", pady=2)
        
        tk.Button(selection_frame, text="Start Program", command=self.start_program).pack(pady=10)
        tk.Button(selection_frame, text="Stop Program", command=self.stop_program).pack(pady=5)
        
        self.video_frame = tk.Frame(master, width=800, height=750, bg="black")
        self.video_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.video_label = tk.Label(self.video_frame)
        self.video_label.pack(fill=tk.BOTH, expand=True)

        self.status_label = tk.Label(master, text="Select a program and press 'Start'", bd=1, relief=tk.SUNKEN, anchor=tk.W)
    ...
    def start_program(self):
        ...
        if option == 1:
            video_path = "/home/jetson/obstruction_gpu/mask/maskoff.mp4"
          
            if not os.path.exists(video_path):
                messagebox.showerror("File Error", f"Video file not found at: {video_path}")
                return
            self.cap = cv2.VideoCapture(video_path)
            self.running_thread = threading.Thread(target=self.run_mask_detection)
            self.status_label.config(text="Running Mask Detection (from file)...")
        elif option == 2:
            self.cap = cv2.VideoCapture(0)
            self.running_thread = threading.Thread(target=self.run_dms_events)
            self.status_label.config(text="Running DMS Events (from camera)...")
    ...
    def update_frame(self, frame):
        ...
        self.master.after(self.frame_update_delay, self.update_frame_on_main_thread)

    def update_frame_on_main_thread(self):
        if self.program_running:
            pass
        ...
        with mp_face_detection.FaceDetection(min_detection_confidence=0.3) as face_detection:
            while self.program_running:
                ...
                cv2.putText(frame, label, (30, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0) if label=="Mask is worn" else (0,0,255), 2)
                
                self.master.after(0, lambda f=frame.copy(): self.update_frame(f))
                
                time.sleep(1/30)

        self.program_running = False
        
# ==================== Other DMS Events Code ======================
    def run_dms_events(self):
        monitor = EdgetensorDMMMonitor()
        ...
        while self.program_running:
            ...
            display_frame = monitor.process_frame(frame)
            
            self.master.after(0, lambda f=display_frame.copy(): self.update_frame(f))
...
def mouth_aspect_ratio(landmarks, MOUTH_INNER_TOP, MOUTH_INNER_BOTTOM, MOUTH_OUTER_HORIZONTAL):
    A = dist.euclidean(np.array([landmarks[MOUTH_INNER_TOP[0]].x, landmarks[MOUTH_INNER_TOP[0]].y]), np.array([landmarks[MOUTH_INNER_BOTTOM[0]].x, landmarks[MOUTH_INNER_BOTTOM[0]].y]))
    B = dist.euclidean(np.array([landmarks[MOUTH_INNER_TOP[1]].x, landmarks[MOUTH_INNER_TOP[1]].y]), np.array([landmarks[MOUTH_INNER_BOTTOM[1]].x, landmarks[MOUTH_INNER_BOTTOM[1]].y]))
    C = dist.euclidean(np.array([landmarks[MOUTH_INNER_TOP[2]].x, landmarks[MOUTH_INNER_TOP[2]].y]), np.array([landmarks[MOUTH_INNER_BOTTOM[2]].x, landmarks[MOUTH_INNER_BOTTOM[2]].y]))
    D = dist.euclidean(np.array([landmarks[MOUTH_OUTER_HORIZONTAL[0]].x, landmarks[MOUTH_OUTER_HORIZONTAL[0]].y]), np.array([landmarks[MOUTH_OUTER_HORIZONTAL[1]].x, landmarks[MOUTH_OUTER_HORIZONTAL[1]].y]))
    mar = (A + B + C) / (3.0 * D)
    return mar
...
class EdgetensorDMMMonitor:
    def __init__(self, config_file='config.ini'):
        ...
        self.roi_position = self.config.get('DEFAULT', 'active_roi')
        self.roi_x1, self.roi_y1, self.roi_x2, self.roi_y2 = self.get_roi_coordinates(self.roi_position)
        ...
        self.voice_alerts = {
            'yawning': 'yawning.mp3', 'distraction': 'distraction.mp3', 'fatigue': 'fatigue.mp3',
            'camera_obstruction': 'obstruction.mp3', 'face_obstruction': 'face_obstruction.mp3',
            'smoking': 'smoking.mp3', 'phone_uses': 'phone_uses.mp3', 'eating_drinking': 'eating_drinking.mp3',
            'seatbelt_off': 'seatbelt_off.mp3', 'left_eye_closed': 'left_eye_closed.mp3',
            'right_eye_closed': 'right_eye_closed.mp3', 'wearing_mask': 'wearing_mask.mp3',
        }
        self.alert_priorities = {
            'camera_obstruction': 1, 'face_obstruction': 2, 'fatigue': 3, 'yawning': 4,
            'distraction': 5, 'phone_uses': 6, 'smoking': 7, 'eating_drinking': 8,
            'wearing_mask': 9, 'seatbelt_off': 10, 'left_eye_closed': 11, 'right_eye_closed': 12,
        }
        ...
        self.events = {
            ...
            'wearing_mask': {'active': False, 'timer': 0.0, 'threshold': 1.0, 'alert_time': 0.0},
    ...
    def load_config(self, config_file):
        ...
        if not os.path.exists(config_file):
            print(f"Error: Config file '{config_file}' not found. Using defaults.")
            config['THRESHOLDS'] = {'ear_threshold': '0.2', 'yawn_threshold': '0.7', 'brightness_threshold': '100', 'color_var_threshold': '10', 'left_threshold': '15', 'right_threshold': '15', 'up_threshold': '15', 'down_threshold': '15'}
            config['TIMERS'] = {'yawn_time': '2', 'fatigue_time': '3', 'distraction_time': '3', 'face_obstruction_time': '2', 'camera_obstruction_time': '2', 'smoking_time': '3', 'phone_use_time': '3', 'eating_drinking_time': '3', 'alert_delay_seconds': '5'}
            config['DEFAULT'] = {'active_roi': '1'}
            config['ROI_1'] = {'x1': '200', 'y1': '150', 'x2': '600', 'y2': '600'}
    ...
    def play_priority_alert(self):
        current_time = time.time()
        active_alerts = []
        for name, data in self.events.items():
            if data['active'] and name in self.alert_priorities and name in self.voice_alerts:
                active_alerts.append((self.alert_priorities[name], name))
        active_alerts.sort(key=lambda x: x[0])

        if not active_alerts:
            self.last_alert_played = None
            return

        highest_priority_event = active_alerts[0][1]
        if highest_priority_event != self.last_alert_played:
            self.play_alert(highest_priority_event)
            self.last_alert_played = highest_priority_event
            self.events[highest_priority_event]['alert_time'] = current_time
        elif (current_time - self.events[highest_priority_event]['alert_time']) > self.ALERT_DELAY_SECONDS:
            self.play_alert(highest_priority_event)
            self.events[highest_priority_event]['alert_time'] = current_time
    ...
    def update_event_states(self, yawn_detected, distraction_detected_from_head_pose, head_direction, left_eye_closed, right_eye_closed, is_camera_obstructed, faces_detected):
        ...
        # Helper function for timer-based events
        def update_timer_event(event_name, condition, start_time_var, threshold):
            if condition:
                if getattr(self, start_time_var) is None: 
                    setattr(self, start_time_var, current_time)
                self.events[event_name]['timer'] = current_time - getattr(self, start_time_var)
                self.events[event_name]['active'] = self.events[event_name]['timer'] >= threshold
            else:
                setattr(self, start_time_var, None)
                self.events[event_name]['timer'] = 0.0
                self.events[event_name]['active'] = False

        # Reset non-continuous events at the beginning of the frame if they are no longer active
        for event_name in self.events:
            if event_name not in ['fatigue', 'yawning', 'distraction', 'phone_uses', 'eating_drinking', 'smoking', 'face_obstruction', 'camera_obstruction']:
                self.events[event_name]['active'] = False
                self.events[event_name]['timer'] = 0.0

        # Camera obstruction takes the highest priority
        update_timer_event('camera_obstruction', is_camera_obstructed, 'camera_obstructed_start_time', self.CAMERA_OBSTRUCTION_TIME)
        if self.events['camera_obstruction']['active']:
            return
        
        # Face obstruction is next highest priority
        update_timer_event('face_obstruction', faces_detected == 0, 'face_obstructed_start_time', self.FACE_OBSTRUCTION_TIME)
        if self.events['face_obstruction']['active']:
            return
        
        # If no obstructions, evaluate other events
        update_timer_event('fatigue', left_eye_closed and right_eye_closed, 'fatigue_start_time', self.FATIGUE_TIME_THRESHOLD)
        update_timer_event('yawning', yawn_detected, 'yawning_start_time', self.YAWN_TIME_THRESHOLD)
        update_timer_event('distraction', distraction_detected_from_head_pose, 'distraction_start_time', self.DISTRACTION_TIME_THRESHOLD)
        
        # Handle non-timer-based events
        self.events['left_eye_closed']['active'] = left_eye_closed
        self.events['right_eye_closed']['active'] = right_eye_closed
        # Placeholder for other events, add logic here as needed
        ...
        self.events['seatbelt_off']['active'] = False
    ...
    def process_frame(self, frame):
        ...
        is_camera_obstructed = self.check_camera_obstruction(frame)
        self.face_not_detected = False
        ...
        cv2.putText(frame, "ROI", (self.roi_x1, self.roi_y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        
        if not is_camera_obstructed and results.multi_face_landmarks:
            self.face_not_detected = False
            for face_landmarks in results.multi_face_landmarks:
                ...
                distraction_detected, direction, self.head_pose_yaw, self.head_pose_pitch, self.head_pose_roll = self.detect_head_pose(face_landmarks.landmark)
                
        self.update_event_states(yawn_detected, distraction_detected, direction, left_eye_closed, right_eye_closed, is_camera_obstructed, faces_detected)

        display_frame[:self.video_height, :self.video_width] = cv2.resize(frame, (self.video_width, self.video_height))

        self.draw_dms_events_panel(display_frame)
        self.draw_dms_output_panel(display_frame)

        self.play_priority_alert()

        frame_end_time = time.time()
        self.metrics['frame_processing_time'] = frame_end_time - frame_start_time
        self.metrics['fps'] = 1 / self.metrics['frame_processing_time'] if self.metrics['frame_processing_time'] > 0 else 0
