...
EAR_THRESHOLD = 0.25
YAWN_THRESHOLD = 0.4  # New Yawn threshold from your logic
YAWN_TIME_THRESHOLD = 0.5  
FATIGUE_TIME = 3.0
DISTRACTION_TIME_THRESHOLD = 5.0
DISTRACTION_ANGLE_THRESHOLD = 20 
...
class EdgetensorDMMMonitor:
    ...
    # --- Updated Mouth Aspect Ratio (MAR) calculation based on the new logic ---
    def mouth_aspect_ratio(self, landmarks):
        """Calculate mouth aspect ratio for yawn detection using MediaPipe landmarks."""
        # Calculate vertical distances for inner lips
        # Using specific MediaPipe landmarks that approximate the vertical opening of the mouth
    ...
    # --- Updated Detect Yawn function based on the new logic ---
    def detect_yawn(self, landmarks):
        ...
        # Confidence calculation (scaled to 1.0 max)
    ...
    def check_camera_obstruction(self, frame):
        """
        Check for camera obstruction based on brightness and color variance.
        A low mean intensity indicates a dark obstruction.
        A low color variance indicates a uniform, featureless obstruction.
        """
        gray_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        mean_intensity = np.mean(gray_frame)
        color_variance = np.std(frame)  # Calculates standard deviation of all channels
    ...
    def update_event_states(self, yawn_detected, distraction_detected_from_head_pose, head_direction, left_eye_closed, right_eye_closed, is_camera_obstructed, faces_detected):
        ...
        if left_eye_closed and right_eye_closed:
            if self.fatigue_start_time is None: self.fatigue_start_time = time.time()
            elif (time.time() - self.fatigue_start_time) >= FATIGUE_TIME:
                self.events['fatigue']['active'] = True
                self.events['fatigue']['timer'] = time.time() - self.fatigue_start_time
        else:
            self.fatigue_start_time = None
    ...
    def draw_dms_events_panel(self, frame):
        ...
        for i, (event_name, label) in enumerate(events_list):
            row, col = i // 5, i % 5
            icon_x, icon_y = panel_x + 20 + col * icon_spacing_x, icon_start_y + row * row_spacing
            is_active = self.events.get(event_name, {'active': False})['active']
            ...
            self.draw_icon(frame, event_name, icon_x, icon_y, bg_color)
            ...
            if not is_camera_obstructed:
                ...
                if results.multi_face_landmarks:
                    ...
                    for face_landmarks in results.multi_face_landmarks:
                        mp_drawing.draw_landmarks(image=video_resized, landmark_list=face_landmarks, connections=mp_face_mesh.FACEMESH_TESSELATION, landmark_drawing_spec=None, connection_drawing_spec=mp_drawing.DrawingSpec(color=(0, 255, 0), thickness=1, circle_radius=1))
